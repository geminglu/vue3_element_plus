...
const avatar = computed(() => userStore.userInfo?.avatars);
</script>

<template>
  <template v-if="image">
    <el-avatar fit="cover" v-if="isString(avatar) && avatar.length > 0" :src="avatar" />
    <el-avatar fit="cover" v-else :src="defaultAvatar" />
  </template>
